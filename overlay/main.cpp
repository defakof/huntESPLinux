/*
 * Hunt ESP Overlay
 * X11/OpenGL/ImGui transparent overlay. Reads game memory via /dev/hunt_read.
 */
...
static std::vector<char> g_selected;
...
/* World-to-screen: view transform then projection */
...
static void draw_esp(const GameState &state) {
    ...
    for (const auto &ent : state.entities) {
        ...
        draw->AddRect(ImVec2(x1 - 1, y1 - 1), ImVec2(x2 + 1, y2 + 1),
                      IM_COL32(0, 0, 0, 200), 0, 0, 1.0f);
        draw->AddRect(ImVec2(x1, y1), ImVec2(x2, y2), color, 0, 0, 1.5f);

        /* Head dot - disabled, bone offsets need work
        ...
        */
        ...
        /* Snap line from screen bottom-center to entity */
...
static void draw_menu(const GameState &state) {
    ...
    if (ImGui::BeginTabBar("Tabs")) {
        if (ImGui::BeginTabItem("Settings")) {
            ...
            ImGui::SameLine();
            ImGui::Checkbox("Raw names##h", &g_settings.raw_names_hunters);
            ImGui::Checkbox("Show Bosses", &g_settings.show_bosses);
            ImGui::SameLine();
            ImGui::Checkbox("Raw names##b", &g_settings.raw_names_bosses);
            ImGui::Checkbox("Show Grunts", &g_settings.show_grunts);
            ImGui::SameLine();
            ImGui::Checkbox("Raw names##g", &g_settings.raw_names_grunts);
            ImGui::Separator();
            ImGui::Checkbox("Show ALL entities (unfiltered)", &g_settings.show_all);

            ImGui::SeparatorText("Colors");
            ImGui::ColorEdit4("Hunter##col", g_settings.color_hunter,
                              ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar);
            ImGui::ColorEdit4("Boss##col", g_settings.color_boss,
                              ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar);
            ImGui::ColorEdit4("Grunt##col", g_settings.color_grunt,
                              ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar);
            ImGui::ColorEdit4("Unknown##col", g_settings.color_unknown,
                              ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar);
        ...
        if (ImGui::BeginTabItem("Entities")) {
            ...
            if (ImGui::BeginTable("ent_table", 4,
                ...
                for (size_t i = 0; i < state.entities.size(); i++) {
                    const auto &ent = state.entities[i];
                    ImGui::TableNextRow();
                    ...
                    bool selected = (i < g_selected.size()) ? (bool)g_selected[i] : false;
                    if (ImGui::Selectable(sel_id, selected,
                            ImGuiSelectableFlags_SpanAllColumns |
                            ImGuiSelectableFlags_AllowOverlap)) {
                        ImGuiIO &sio = ImGui::GetIO();
                        if (sio.KeyShift && g_last_clicked >= 0) {
                            int lo = std::min(g_last_clicked, (int)i);
                            int hi = std::max(g_last_clicked, (int)i);
                            for (int r = lo; r <= hi; r++)
                                g_selected[r] = true;
                        } else if (sio.KeyCtrl) {
                    ...
                    ImGui::TextColored(colv, "%s",
                                       ent.raw_name[0] ? ent.raw_name : ent.name);
...
/* Toggle X11 input passthrough for click-through overlay */
...
int main(int argc, char **argv) {
    ...
    /* Override-redirect: bypass WM stacking so overlay renders above fullscreen */
    ...
    while (!glfwWindowShouldClose(window) && g_running) {
        ...
        /* Global hotkey: Insert toggles menu (works without window focus) */
        char keymap[32] = {};
        ...
        /* Re-raise overlay periodically to stay on top */

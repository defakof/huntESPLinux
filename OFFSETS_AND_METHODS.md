...
1. A `float` in range `0.4 — 2.0` (FOV in radians, ~78° default = 1.36 rad)
...
| `+0x00` | `float[3]` | Row 0: right vector (r0,r1,r2) + pos.x |
| `+0x10` | `float[3]` | Row 1: forward vector (u0,u1,u2) + pos.y |
| `+0x20` | `float[3]` | Row 2: up vector (f0,f1,f2) + pos.z |
| `+0x30` | `float` | FOV in radians |
...
- **Rotation**: Updates but may lag 1-2 frames behind actual view
- The camera data in CryRenderD3D12.dll .data is written by the render thread
- For pixel-perfect W2S, you need the game's actual `GetRenderMatrix()` and `GetProjectionMatrix()` 4x4 matrices — these are NOT easily accessible from .data section

### World-to-Screen (Reference from Game Code)
```c
// The game uses TWO separate matrices:
// 1. RenderMatrix (4x4 view matrix) — from GetRenderMatrix()
// 2. ProjectionMatrix (4x4 projection) — from GetProjectionMatrix()
...
### W2S — UNSOLVED ISSUES
The view matrix (RenderMatrix) must be the actual game view matrix, not computed from the camera Matrix34. Computing it from the Matrix34 produces incorrect results because:
1. The Matrix34 rotation lags behind the actual view
2. The CryEngine Matrix34 row ordering (right/forward/up) mapping to view-space axes needs to be exact

**TODO**: Find the actual `GetRenderMatrix()` and `GetProjectionMatrix()` return values in memory. They're likely stored in a heap-allocated `CRenderView` object, not in the renderer's .data section. An IDA script to decompile `GetRenderMatrix()` would reveal the exact offset.

/*
 * hunt_reader.ko - Kernel module for reading game process memory
 *
 * Provides /dev/hunt_read character device with ioctl interface
 * for reading memory from a target Wine/Proton process.
 *
 * Usage:
 *   1. Load module: sudo insmod hunt_reader.ko
 *   2. Set target PID: ioctl(fd, HUNT_IOC_SET_PID, &pid)
 *   3. Find module base: ioctl(fd, HUNT_IOC_GET_MODULE, &module_req)
 *   4. Read memory: ioctl(fd, HUNT_IOC_READ_MEM, &read_req)
 */
...
/*
 * Read memory from target process using access_process_vm.
 * This is the proper kernel API for cross-process memory reads.
 */
...
/*
 * Find a memory-mapped module in the target process by name.
 * Walks the VMA list looking for file-backed mappings matching the name.
 * Returns the base address and size of the first matching mapping.
 */
static int hunt_get_module(struct hunt_module_req __user *ureq)
{
    ...
    /*
     * Wine/Proton maps DLLs as multiple non-contiguous VMAs (separate
     * sections with gaps between them). Walk all VMAs to find every
     * mapping of the same file and compute the full span.
     */
    {
        ...
        for_each_vma(vmi, vma) {
            ...
            if (!match_dentry) {
                ...
                /* Subsequent: collect all VMAs of same file */
...
out:
    ...
    return ret == 0 ? 0 : 0; /* always return 0, result is in req.result */
}
...
static struct miscdevice hunt_misc = {
    ...
    .fops  = &hunt_fops,
    .mode  = 0666, /* world accessible - overlay runs as user */
};
